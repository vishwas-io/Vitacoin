...
func (suite *KeeperTestSuite) TestAccrueDelegatorReward() {
	...
	err := suite.keeper.AccrueDelegatorReward(suite.ctx, delegator, amount, "avita")
	...
	suite.Require().Equal("avita", denom)
}
...
func (suite *KeeperTestSuite) TestAccrueDelegatorReward_Accumulates() {
	...
	err := suite.keeper.AccrueDelegatorReward(suite.ctx, delegator, first, "avita")
	suite.Require().NoError(err)
	err = suite.keeper.AccrueDelegatorReward(suite.ctx, delegator, second, "avita")
	suite.Require().NoError(err)
